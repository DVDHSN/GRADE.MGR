      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card title="Subject Band Distribution">
            <div className="relative h-full min-h-[300px]">
                ...
                <div className="absolute bottom-2 right-2 text-[10px] text-zinc-600 font-mono text-right">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <Card title="Performance Extremes">
            <div className="flex flex-col gap-6 h-full justify-center">
                <div>
                    <h4 className="text-[10px] font-mono uppercase text-emerald-500 mb-2 tracking-widest">Dominating</h4>
                    {strongest.length > 0 ? strongest.map((s, i) => {
                        const repGrade = grades.find(g => g.courseName === s.name);
                        return (
                        <div key={s.name} className="flex justify-between border-b border-zinc-900 py-1 last:border-0 hover:bg-zinc-900/50 hover:pl-2 transition-all duration-300 px-1 cursor-default group">
                            <span className="font-bold text-zinc-300 text-sm group-hover:text-emerald-300 transition-colors">
                              {getSubjectCode(s.name, repGrade?.customCode)} 
                              <span className="text-[10px] text-zinc-600 font-normal ml-2 hidden lg:inline group-hover:text-emerald-500/70">{s.name.slice(0,10)}...</span>
                            </span>
                            <span className="font-mono text-emerald-500 font-bold">{s.avg}%</span>
                        </div>
                    )}) : <span className="text-zinc-700 text-xs">N/A</span>}
                </div>
                <div>
                    <h4 className="text-[10px] font-mono uppercase text-red-500 mb-2 tracking-widest">Critical Focus</h4>
                    {weakest.length > 0 ? weakest.map((s, i) => {
                        const repGrade = grades.find(g => g.courseName === s.name);
                        return (
                        <div key={s.name} className="flex justify-between border-b border-zinc-900 py-1 last:border-0 hover:bg-zinc-900/50 hover:pl-2 transition-all duration-300 px-1 cursor-default group">
                            <span className="font-bold text-zinc-300 text-sm group-hover:text-red-300 transition-colors">
                              {getSubjectCode(s.name, repGrade?.customCode)}
                              <span className="text-[10px] text-zinc-600 font-normal ml-2 hidden lg:inline group-hover:text-red-500/70">{s.name.slice(0,10)}...</span>
                            </span>
                            <span className="font-mono text-red-500 font-bold">{s.avg}%</span>
                        </div>
                    )}) : <span className="text-zinc-700 text-xs">N/A</span>}
                </div>
            </div>
      ...
      <Card title="Detailed Subject Breakdown">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {subjectDetails.map(({ subject, typeStats, customCode }) => (
                <div key={subject} className="bg-zinc-900/30 border border-zinc-900 p-4 hover:border-zinc-700 transition-all duration-300 group hover:-translate-y-1 hover:shadow-xl hover:bg-zinc-900/50 hover:scale-[1.01] transform-gpu">
                    <div className="flex justify-between items-baseline mb-3 border-b border-zinc-800 pb-2">
                        <div className="flex items-center gap-2">
                           <span className="text-2xl font-black text-white group-hover:text-red-500 transition-colors">{getSubjectCode(subject, customCode)}</span>
                           <span className="text-[10px] text-zinc-600 font-mono uppercase truncate max-w-[120px]">{subject}</span>
                        </div>
                    </div>
                        ...
                        {typeStats.map(stat => (
                            <div key={stat.type} className="flex justify-between items-center text-xs">
                                <span className="uppercase font-mono text-zinc-500 flex items-center gap-2">
                                    <span className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[stat.type] }}></span>
                                    {stat.type}
                                </span>
                                <span className="font-mono font-bold text-zinc-300">{stat.avg}%</span>
                            </div>
                        ))}
            ...
            {subjectDetails.length === 0 && (
                <div className="col-span-full py-8 text-center text-zinc-600 font-mono text-xs uppercase tracking-widest">
